...
export async function GET(req: Request) {
  ...
  try {
    ...
    page7.drawText('The subject belongs to an elite tier of psychological profiles capable of sustaining professional-grade neural load.', { x: 70, y: 430, size: 10, font: fontRegular, color: COLORS.white, maxWidth: 460 });
    ...
    page8.drawText('SCAN_AUTHENTICITY_CONFIRMED', { x: width/2 - 120, y: 400, size: 12, font: fontBold, color: COLORS.neon });
    page8.drawText(`CERTIFICATE_ID: ${sessionId?.substring(0, 12).toUpperCase()}`, { x: width/2 - 120, y: 380, size: 8, font: fontBold, color: COLORS.white });
    
    page8.drawText('NO_HUMAN_MODIFICATION_DETECTED', { x: width/2 - 120, y: 360, size: 8, font: fontBold, color: COLORS.gray });
    
    page8.drawText('RECOMMENDED_RESCAN_DATE: 2026-06-22', { x: 50, y: 150, size: 10, font: fontBold, color: COLORS.white });
    
    // Final Footer
    page8.drawText('CONFIDENTIAL PLAYERDNA DOSSIER // GENERATED BY NEURAL_ENGINE_S26', { x: 0, y: 50, size: 8, font: fontBold, color: COLORS.gray, width: 595, align: 'center' as any });
