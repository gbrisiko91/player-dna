...
import { Share2, Download, Trophy, Target, Zap, Shield, Brain, Fingerprint, Loader2 } from "lucide-react";
...
export default function ResultCard({ archetype }: ResultCardProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      // Aspettiamo un attimo per assicurarci che i font siano caricati e le animazioni stabilizzate
      const dataUrl = await toPng(cardRef.current, { 
        cacheBust: true,
        quality: 1,
        pixelRatio: 2 // Alta risoluzione per i social
      });
